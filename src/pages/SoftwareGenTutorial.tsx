...
export default function SoftwareGenTutorial() {
  const { lang } = useT()
  const l = {
    pageTitle: lang === 'zh-CN' ? '桌面软件 生成教程' : lang === 'zh-TW' ? '桌面軟體 生成教學' : 'AI Software/EXE Generation — Complete Guide',
    pageSub: lang === 'zh-CN' ? '从想法到桌面软件 · AI编程全流程' : lang === 'zh-TW' ? '從想法到可執行文件 · AI編程全流程' : 'From Idea to Executable · Full AI Coding Pipeline',
    whatIs: lang === 'zh-CN' ? '桌面软件生成概览' : lang === 'zh-TW' ? '桌面軟體生成概覽' : 'Overview',
