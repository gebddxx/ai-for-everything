  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.chart} style={{marginBottom:20}}>
        <div style={{height:1,background:'var(--border)',margin:'36px 0 20px'}} /><h3 style={{fontSize:17,fontWeight:700,color:'var(--text)',marginBottom:16,display:'flex',alignItems:'center',gap:8}}>{L('AI安全正在改变什么','AI is Transforming Cybersecurity','AI正在改變網路安全')}</h3>
      ...
      <section className={styles.chart} style={{marginBottom:20}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:12}}>
          {[{icon:'🛡️',t:L('AI威胁检测','Threat Detection','AI威脅檢測'),d:L:'AI分析网络流量和行为模式实时发现异常。Darktrace的自主学习AI无需规则即可检测零日攻击。CrowdStrike Falcon用AI每天处理数万亿事件，将MTTD（平均检测时间）从数天压缩到1分钟。Vectra AI的Attack Signal Intelligence将告警准确率提升至98%。','AI analyzes network traffic and behavior for real-time anomaly detection. Darktrace self-learning AI detects zero-days without rules. CrowdStrike Falcon processes trillions of events daily, compressing MTTD from days to 1 minute. Vectra AI Attack Signal Intelligence achieves 98% alert accuracy.','AI分析網路流量和行為模式即時發現異常。Darktrace的自學習AI無需規則即可檢測零日攻擊。CrowdStrike Falcon用AI每天處理數兆事件。')},
          {icon:'📧',t:L('AI反钓鱼与欺诈','Anti-Phishing & Fraud','AI反釣魚與欺詐'),d:L:'AI识别GPT生成的钓鱼邮件，准确率超99%。Abnormal Security用AI分析邮件上下文和行为基线，拦截了传统安全工具漏掉的80%的精准钓鱼攻击。中国微步在线用AI威胁情报服务金融和政府客户。','AI detects GPT-generated phishing emails with 99%+ accuracy. Abnormal Security uses AI to analyze email context and behavioral baselines, blocking 80% of targeted phishing missed by traditional tools. Chinese ThreatBook provides AI threat intelligence.','AI識別GPT生成的釣魚郵件，準確率超99%。Abnormal Security用AI分析郵件上下文和行為基線，攔截了傳統安全工具漏掉的80%的精準釣魚攻擊。')},
          {icon:'🤖',t:L('AI自动化响应(SOAR)','Automated Response','AI自動化響應'),d:L:'AI在检测到威胁后自动执行隔离、阻断和修复。Palo Alto Cortex XSOAR用AI编排安全响应剧本，将事件处理时间从小时缩短到秒。Google Chronicle的AI安全分析每秒处理PB级数据。','AI automatically isolates, blocks and remediates after threat detection. Palo Alto Cortex XSOAR orchestrates security response playbooks with AI, reducing incident handling from hours to seconds. Google Chronicle AI Security Analytics processes petabytes per second.','AI在檢測到威脅後自動執行隔離、阻斷和修復。Palo Alto Cortex XSOAR用AI編排安全響應劇本，將事件處理時間從小時縮短到秒。')},
          {icon:'🔑',t:L('AI身份安全','Identity Security','AI身份安全'),d:L:'AI分析登录行为检测账号盗用。Microsoft Entra用AI每天阻止数亿次身份攻击，将MFA疲劳攻击减少90%。Okta的AI身份引擎实现自适应认证，高风险行为自动触发二次验证。','AI analyzes login behavior to detect account takeover. Microsoft Entra blocks hundreds of millions of identity attacks daily with AI, reducing MFA fatigue by 90%. Okta AI identity engine enables adaptive authentication with automatic step-up for risky behavior.','AI分析登錄行為檢測帳號盜用。Microsoft Entra用AI每天阻止數億次身份攻擊。Okta的AI身份引擎實現自適應認證。')},
      ...
      <section className={styles.chart} style={{marginBottom:20}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))',gap:10}}>
          {[{v:'秒级',l:L('AI威胁检测+隔离','Detection+Isolation','AI威脅檢測+隔離')},{v:'99%+',l:L('AI识别GenAI钓鱼','GenAI Phishing Detect','AI識別GenAI釣魚')},{v:'90%',l:L('MFA攻击减少','MFA Attack Drop','MFA攻擊減少')},{v:'$60B+',l:L('2028年AI安全市场','2028 AI Security Mkt','2028年AI安全市場')}].map(m => (<div key={m.l} style={{textAlign:'center',padding:'16px 12px',background:'var(--bg-card)',borderRadius:10,border:'1px solid var(--border)'}}><div style={{fontSize:24,fontWeight:800,color:'var(--primary)'}}>{m.v}</div><div style={{fontSize:12,color:'var(--text-secondary)',marginTop:4}}>{m.l}</div></div>))}
        </div>
      </section>
