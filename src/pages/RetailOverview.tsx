  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.chart} style={{marginBottom:20}}>
        <div style={{height:1,background:'var(--border)',margin:'36px 0 20px'}} /><h3 style={{fontSize:17,fontWeight:700,color:'var(--text)',marginBottom:16,display:'flex',alignItems:'center',gap:8}}>{L('AI零售正在改变什么','AI is Transforming Retail','AI正在改變零售')}</h3>
      ...
      <section className={styles.chart} style={{marginBottom:20}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:12}}>
          {[{icon:'🎯',t:L('AI个性化推荐','Recommendation','AI個性化推薦'),d:L('深度学习分析用户行为，实时生成千人千面的商品推荐。亚马逊35%的销售来自AI推荐，淘宝推荐系统每天处理100亿+次请求。TikTok Shop的AI算法将商品发现转化率提升300%。','Deep learning generates personalized product recommendations in real-time. Amazon gets 35% sales from AI recs, Taobao handles 10B+ daily requests. TikTok Shop AI boosts product discovery conversion by 300%.','深度學習分析用戶行為，即時生成千人千面的商品推薦。亞馬遜35%銷售來自AI推薦，淘寶推薦系統每天處理100億+次請求。')},
          {icon:'📊',t:L('AI动态定价','Dynamic Pricing','AI動態定價'),d:L('AI根据库存、竞品价格、需求预测实时调整价格。Walmart的AI定价系统每天调整数百万SKU。Uber的激增定价算法平衡供需，高峰期接单率提升40%。中国美团用AI实时优化外卖配送费和菜品定价。','AI adjusts prices in real-time based on inventory, competitor pricing and demand. Walmart AI reprices millions of SKUs daily. Uber surge pricing balances supply-demand, boosting peak-hour fulfillment by 40%. Meituan optimizes delivery fees dynamically.','AI根據庫存、競品價格、需求預測即時調整價格。Walmart的AI定價系統每天調整數百萬SKU。')},
          {icon:'🏪',t:L('AI无人零售','Autonomous Retail','AI無人零售'),d:L('计算机视觉+传感器实现无收银员购物。Amazon Go已开设20+家无人店，AI追踪顾客取放商品自动结算。中国缤果盒子、小麦铺等无人零售曾快速扩张。AI视觉收银台将结账速度提升5倍。','CV + sensors enable cashierless shopping. Amazon Go operates 20+ stores where AI tracks item pick-up and auto-charges. Chinese BingoBox and Xiaomaipu pioneered unmanned retail. AI vision checkout boosts speed 5x.','計算機視覺+傳感器實現無收銀員購物。Amazon Go已開設20+家無人店。AI視覺收銀台將結帳速度提升5倍。')},
          {icon:'📦',t:L('AI供应链与库存','Supply Chain & Inventory','AI供應鏈與庫存'),d:L('AI预测区域需求，优化补货和仓储布局。Zara用AI分析全球门店销售数据，将库存周转缩短至2周。盒马鲜生AI将生鲜损耗率从行业平均20%降至7%。Shein的AI供应链将新款从设计到上架压缩到3天。','AI predicts regional demand, optimizes replenishment and warehouse layout. Zara uses AI to analyze global store sales, reducing inventory turnover to 2 weeks. Hema AI cuts fresh food spoilage from 20% to 7%. Shein AI supply chain compresses design-to-shelf to 3 days.','AI預測區域需求，優化補貨和倉儲佈局。Zara用AI分析全球門店銷售數據。Shein的AI供應鏈將新款從設計到上架壓縮到3天。')},
      ...
      <section className={styles.chart} style={{marginBottom:20}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))',gap:10}}>
          {[{v:'35%',l:L('亚马逊AI推荐贡献营收','Amazon AI Rec Revenue','亞馬遜AI推薦貢獻')},{v:'↓30%',l:L('AI生鲜损耗降低','Fresh Food Spoilage Drop','AI生鮮損耗降低')},{v:'5×',l:L('AI结账速度提升','AI Checkout Speed','AI結帳速度提升')},{v:'400-800B',l:L('AI零售价值潜力','AI Retail Value','AI零售價值潛力')}].map(m => (<div key={m.l} style={{textAlign:'center',padding:'16px 12px',background:'var(--bg-card)',borderRadius:10,border:'1px solid var(--border)'}}><div style={{fontSize:24,fontWeight:800,color:'var(--primary)'}}>{m.v}</div><div style={{fontSize:12,color:'var(--text-secondary)',marginTop:4}}>{m.l}</div></div>))}
        </div>
      </section>
