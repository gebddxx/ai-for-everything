...
export default function TransportOverview() {
  ...
  return (
    <div className={styles.page}>
      <div className={styles.head}><h2 className={styles.title}>🚗 {L('AI 交通出行','AI in Transportation','AI 交通出行')}</h2><p className={styles.subtitle}>{L('AI如何驅動自動駕駛、智慧交通和出行服務','How AI powers autonomous driving, smart traffic, and mobility services','AI如何驱动自动驾驶、智慧交通和出行服务')}</p></div>
      <section className={styles.chart} style={{marginBottom:20}}>
        <div style={{height:1,background:'var(--border)',margin:'36px 0 20px'}} /><h3 style={{fontSize:17,fontWeight:700,color:'var(--text)',marginBottom:16,display:'flex',alignItems:'center',gap:8}}>{L('AI正在改變交通','AI交通正在改变什么','AI is Transforming Transportation')}</h3>
      ...
      <section className={styles.chart} style={{marginBottom:20}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:12}}>
          {[{icon:'🚕',t:L('無人計程車','无人出租车','Robotaxi'),d:L('Waymo在舊金山/鳳凰城/洛杉磯每週完成15萬+次付費出行。百度蘿蔔快跑在武漢運營超500輛，累計訂單超600萬。','Waymo在旧金山/凤凰城/洛杉矶每周完成15万+次付费出行，2025年扩展到奥斯汀和亚特兰大。百度萝卜快跑在武汉运营超500辆，累计订单超600万。Cruise因安全事件暂停运营后重启。','Waymo completes 150K+ paid rides weekly across SF/Phoenix/LA, expanding to Austin and Atlanta in 2025. Baidu Apollo Go operates 500+ vehicles in Wuhan, 6M+ cumulative orders. Cruise resumed operations after safety pause.')},
          {icon:'🧠',t:L('端到端自動駕駛','端到端自动驾驶','End-to-End Autonomy'),d:L('特斯拉FSD V12完全用神經網絡替代規則代碼，訓練數據超10億英里。華為ADS 3.0實現城市NOA不依賴高精地圖，覆蓋全國400+城市。','特斯拉FSD V12完全用神经网络替代规则代码，训练数据超10亿英里。Wayve的LINGO-2用自然语言解释驾驶决策。华为ADS 3.0实现城市NOA不依赖高精地图，覆盖全国400+城市。','Tesla FSD V12 replaces rule-based code entirely with neural networks, trained on 1B+ miles. Wayve LINGO-2 explains driving decisions in natural language. Huawei ADS 3.0 achieves city NOA without HD maps, covering 400+ Chinese cities.')},
          {icon:'🚦',t:L('AI智慧交通','AI智慧交通','Smart Traffic'),d:L('AI即時分析路口攝像頭數據動態調整紅綠燈。杭州城市大腦將通行效率提升15%。Google Green Light項目用AI優化70+城市交通信號。','AI实时分析路口摄像头数据动态调整红绿灯。杭州城市大脑将通行效率提升15%，救护车到达时间缩短50%。Google Green Light项目用AI优化70+城市交通信号，减少30%停车启动。','AI analyzes intersection cameras in real-time to dynamically adjust traffic lights. Hangzhou City Brain improved traffic efficiency by 15%, ambulance arrival time by 50%. Google Green Light optimizes signals in 70+ cities, reducing stops by 30%.')},
          {icon:'🛵',t:L('AI出行與配送','AI出行与配送','Mobility & Delivery'),d:L('滴滴AI調度系統每天匹配數千萬訂單，將乘客等待時間縮短35%。美團無人機配送在深圳完成10萬+訂單。','滴滴AI调度系统每天匹配数千万订单，将乘客等待时间缩短35%。美团无人机配送在深圳完成10万+订单。Uber用AI预测需求热点，司机空驶率降低20%。','Didi AI dispatch matches tens of millions of rides daily, reducing wait time by 35%. Meituan drones completed 100K+ deliveries in Shenzhen. Uber uses AI to predict demand hotspots, reducing driver empty miles by 20%.')},
      ...
      <section className={styles.chart} style={{marginBottom:20}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))',gap:10}}>
          {[{v:'150K+/周',l:L('Waymo付費出行','Waymo付费出行','Waymo Weekly Rides')},{v:'↓25%',l:L('AI減少城市擁堵','AI减少城市拥堵','Urban Congestion Drop')},{v:'600万+',l:L('百度蘿蔔快跑訂單','百度萝卜快跑累计订单','Baidu Apollo Orders')},{v:'20+城',l:L('中國自動駕駛測試','中国自动驾驶测试城市','China AV Test Cities')}].map(m => (<div key={m.l} style={{textAlign:'center',padding:'16px 12px',background:'var(--bg-card)',borderRadius:10,border:'1px solid var(--border)'}}><div style={{fontSize:24,fontWeight:800,color:'var(--primary)'}}>{m.v}</div><div style={{fontSize:12,color:'var(--text-secondary)',marginTop:4}}>{m.l}</div></div>))}
        </div>
      </section>
