  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.chart} style={{marginBottom:20}}>
        <div style={{height:1,background:'var(--border)',margin:'36px 0 20px'}} /><h3 style={{fontSize:17,fontWeight:700,color:'var(--text)',marginBottom:16,display:'flex',alignItems:'center',gap:8}}>{L('AI is Transforming Media','AI媒体正在改变什么','AI正在改變媒體')}</h3>
      ...
      <section className={styles.chart} style={{marginBottom:20}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:12}}>
          {[{icon:'🎮',t:L('AI Gaming','AI游戏','AI遊戲'),d:L('Generative AI redefines game development. NVIDIA ACE gives NPCs real-time conversational ability. Roblox AI tools let creators generate 3D worlds from natural language. Tencent AI Lab Juewu beat pro players in Honor of Kings. AI game asset generation cuts art costs by 50-70%.','生成式AI正在重新定义游戏开发。NVIDIA ACE用AI为NPC赋予实时对话能力。Roblox的AI工具让创作者用自然语言生成3D世界。腾讯AILab的绝悟AI在王者荣耀中击败职业选手。AI游戏资产生成将美术成本降低50-70%。','生成式AI正在重新定義遊戲開發。NVIDIA ACE用AI為NPC賦予即時對話能力。Roblox的AI工具讓創作者用自然語言生成3D世界。')},
          {icon:'🎥',t:L('Film & Video','AI影视制作','AI影視製作'),d:L('AI輔助劇本分析、特效生成和智慧剪輯。Runway的AI工具已被好萊塢用於《瞬息全宇宙》特效。Adobe Premiere Pro的AI自動生成字幕和場景檢測。','AI辅助剧本分析、特效生成和智能剪辑。Runway的AI工具已被好莱坞用于《瞬息全宇宙》特效。Adobe Premiere Pro的AI自动生成字幕和场景检测。中国博纳影业用AI进行票房预测和营销策略优化，准确率达85%+。','AI assists script analysis, VFX and smart editing. Runway AI tools were used in Hollywood\'s "Everything Everywhere All At Once". Adobe Premiere AI auto-generates captions and scene detection. Chinese Bona Film uses AI for box office prediction at 85%+ accuracy.')},
          {icon:'📰',t:L('News & Content','AI新闻与内容','AI新聞與內容'),d:L('AP generates 3,700 quarterly earnings reports with AI. Washington Post Heliograf AI auto-wrote Olympic event coverage. Toutiao AI generates tens of thousands of articles daily. Bloomberg uses GPT for financial news summaries at 98% accuracy.','美联社用AI每季度自动生成3700篇财报新闻。华盛顿邮报的Heliograf AI在里约奥运会期间自动撰写赛事报道。今日头条AI写作每天生成数万篇资讯。Bloomberg用GPT自动生成金融新闻摘要，准确率98%。','美聯社用AI每季度自動生成3700篇財報新聞。今日頭條AI寫作每天生成數萬篇資訊。Bloomberg用GPT自動生成金融新聞摘要。')},
          {icon:'🎵',t:L('Music & Audio','AI音乐与音频','AI音樂與音頻'),d:L('AI composition and voice synthesis are transforming music. Suno and Udio let anyone generate full songs from text. Universal Music uses AI for copyright detection. Spotify AI DJ generates personalized radio. NetEase Cloud Music AI drives 70% of song discovery.','AI作曲和声音合成正在改变音乐产业。Suno和Udio让普通用户用文字生成完整歌曲。环球音乐用AI检测版权侵权。Spotify的AI DJ根据用户口味生成个性化电台。网易云音乐的AI推荐驱动70%的歌曲发现。','AI作曲和聲音合成正在改變音樂產業。Suno和Udio讓普通用戶用文字生成完整歌曲。Spotify的AI DJ根據用戶口味生成個性化電台。')},
      ...
      <section className={styles.chart} style={{marginBottom:20}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))',gap:10}}>
          {[{v:'$1B',l:L('Netflix Annual AI Save','Netflix AI年省流失成本','Netflix年省流失成本')},{v:'50-70%',l:L('Game Art Cost Drop','AI降低游戏美术成本','AI降低遊戲美術成本')},{v:'3,700/季',l:L('AP AI Articles/Qtr','美联社AI自动写稿','美聯社AI自動寫稿')},{v:'$50B+',l:L('2027 AI Media Market','2027年AI媒体市场','2027年AI媒體市場')}].map(m => (<div key={m.l} style={{textAlign:'center',padding:'16px 12px',background:'var(--bg-card)',borderRadius:10,border:'1px solid var(--border)'}}><div style={{fontSize:24,fontWeight:800,color:'var(--primary)'}}>{m.v}</div><div style={{fontSize:12,color:'var(--text-secondary)',marginTop:4}}>{m.l}</div></div>))}
        </div>
      </section>
